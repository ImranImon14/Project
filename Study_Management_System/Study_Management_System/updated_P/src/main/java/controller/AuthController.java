...
public class AuthController {
    ...
    public static void register(String name, String email, String password, String role,
        ...
        try (Connection conn = DBConnection.getConnection()) {
            ...
            // ❗️Check if course is assigned by a teacher (only for students)
            if (role.equals("student")) {
                String checkAssignedSQL = "SELECT COUNT(*) AS total FROM teacher_courses WHERE course_id = ?";
                try (PreparedStatement ps = conn.prepareStatement(checkAssignedSQL)) {
                    ps.setInt(1, courseId);
                    ResultSet rs = ps.executeQuery();
                    if (rs.next() && rs.getInt("total") == 0) {
                        conn.rollback();
                        JOptionPane.showMessageDialog(frame, "This course is not yet assigned by any teacher.");
                        return;
                    }
                }
            }
